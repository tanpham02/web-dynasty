  ...
  return (
    ...
    >
      ...
      <div className='bg-white flex items-center justify-between gap-2 px-6 py-[11px]'>
        <NavLink to={PATH_NAME.STAFF_MANAGEMENT}>
          <img
            src={Logo}
            alt='Logo'
          />
      ...
      <div className='no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear'>
        ...
        <nav className='mt-5 px-4 py-4 lg:mt-9 lg:px-6'>
          ...
          {routeSideBar.map((item, index) => (
            <div key={index}>
              <h3 className='mb-[0.45rem] ml-4 text-sm font-semibold uppercase text-white'>{item.title}</h3>
                ...
                {item.menu.map((menu, index) => {
                  return menu.child.length > 0 ? (
                    ...
                    >
                      <NavLink
                        to={menu.path}
                        className={`relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-normal text-white duration-300 ease-in-out hover:bg-[#005226] dark:hover:!bg-[#005226] ${
                          pathname === `${menu.path}` && 'bg-[#005226] dark:bg-[#005226]'
                        }`}
