  ...
  const COLUMNS: Columns[] = [
    ...
    },
    {
      ...
      render: (__index, record) => (
        <Typography.Text>{handleGenerateAddress(record)}</Typography.Text>
      ),
      responsive: ['xl'],
    },
    {
      ...
      render: (__index, record) => (
        ...
        >{`${
          record.typeOrder === TypeOrder.ORDER_DELIVERING ? 'Đặt giao hàng' : 'Đặt đến lấy'
        }`}</Typography.Text>
      ),
      responsive: ['md'],
    },
  ...
  return (
    <Table
      ...
      onChange={({ current, pageSize, total }: TablePaginationConfig) => {
        onGetPaginationState({
          pageIndex: current ? current - 1 : current,
          pageSize,
        });
      }}
      locale={{
