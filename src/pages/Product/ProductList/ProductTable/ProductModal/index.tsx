...
function ProductModal({ modalType, visible, onClose, refetchData, product }: ProductModalProps) {
  ...
  const onSubmit = async (dataSubmit: ProductMain) => {
    ...
    try {
      formData.append('files', fileProduct?.fileUpload || '');

      formData.append('productInfo', JSON.stringify(dataSubmit));

      modalType === ModalType.CREATE
        ? await productService.createProduct(formData)
        : modalType === ModalType.UPDATE && product?._id && (await productService.updateProduct(formData, product._id));
      ...
      >
        ...
        >
          <Row gutter={24}>
            ...
            <Col span={16}>
              <Typography.Text className='text-[17px] font-semibold'>Thông tin sản phẩm</Typography.Text>
              <Row
                gutter={16}
                className='mt-5'
              >
                        ...
                        render={({ field: { value, onChange } }) => (
                          <InputNumber
                            ...
                            className={`h-[38px] !w-full   ${
                              errors.price ? '!border-danger border-solid  border-[100%] border-t-0 border-l-0' : ''
                            } `}
                      ...
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <InputNumber
                            ...
                            onChange={(value) => value && onChange(value)}
                            className={`h-[38px] !w-full   ${
                              errors.oldPrice ? '!border-danger border-solid  border-[100%] border-t-0 border-l-0' : ''
                            } `}
                            placeholder='Nhập số tiền được giảm'
                            controls={false}
                            addonAfter='đ'
