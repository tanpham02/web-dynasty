...
const ProductCategoryFromThirdPartyModal = ({
  ...
  const [listProductCategorySelected, setListProductCategorySelected] = useState<ProductCategory[]>([]);

  const {
    data: productCategoryFromThirdPartyResponse,
    refetch,
    isLoading: isLoadingProductCategoryFromThirdPart,
  } = useInfiniteQuery([QUERY_KEY.PRODUCT_CATEGORY_FROM_THIRD_PARTY_IN_PRODUCT_CATEGORY_PAGE], async () => {
  ...
  const productCategoryFromThirdPartyResponseAfterChangeField = useMemo(() => {
    return (
      ...
      changeChildCategoryDTOsToChildren(productCategoryFromThirdPartyResponse?.pages[0])
    );
  ...
  const rowSelection: TableRowSelection<ProductCategory> = {
    ...
    checkStrictly: false,
    renderCell(__checked: boolean, record: ProductCategory, __index: number, node: any) {
  ...
  const handleAddProductCategoryFromThirdPartyToZaloMiniApp = async () => {
    if (listProductCategorySelected.length !== 0 && productCategoryFromThirdPartyResponse) {
      ...
      listProductCategory.map((productCategory) => {
        if (productCategoryFromThirdPartyResponseAfterChangeField) {
          if (productCategory.nhanhVnParentId) {
            const parentProductCategorySelected = productCategoryFromThirdPartyResponseAfterChangeField.find(
              (productCategoryFromThirdParty) =>
                productCategoryFromThirdParty.nhanhVnId === productCategory.nhanhVnParentId &&
            ...
            if (parentProductCategorySelected) {
              ...
              const isAlreadySelected = tempListProductCategory.some(
                (item) => item.nhanhVnId === parentProductCategorySelected.nhanhVnId,
              );
              if (!isAlreadySelected) {
                tempListProductCategory.push(parentProductCategorySelected);
              }
              const filteredListProductCategory = tempListProductCategory.filter(
                (item, index, self) => index === self.findIndex((i) => i.nhanhVnId === item.nhanhVnId),
              );
      ...
      if (listProductCategory) {
        try {
          await productCategoryService.addProductCategoryFromThirdPartyToZaloMiniApp(listProductCategory);
        ...
        <div onClick={(e) => e.stopPropagation()}>
          <div className='!h-full'>
            <div className='flex items-center gap-2 lg:w-[75%] md:w-[75%] py-4 '>
              <span className='font-bold text-xl'>{'Danh sách danh mục sản phẩm được lấy từ NhanhVN'}</span>
            </div>
            ...
            <Table
              ...
              pagination={false}
              locale={{
                emptyText: isLoadingProductCategoryFromThirdPart ? <Skeleton /> : <Empty />,
              }}
              expandable={{ defaultExpandAllRows: true }}
            />
          </div>

          <div className={`flex ${listProductCategorySelected.length > 0 ? 'justify-between' : 'justify-end'} mt-5 `}>
            ...
            <div>
              ...
              <button
                className={`w-auto px-8 py-2 ml-3 rounded-lg font-medium ${
                  listProductCategorySelected.length == 0 ? 'bg-stroke ' : 'bg-menu-color hover:bg-primary'
                }  text-white`}
