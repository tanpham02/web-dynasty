...
export const GeneralConfigsPage = () => {
  ...
  const onSubmitSystemConfig = async (systemConfigServiceFormData: any) => {
    if (systemConfigServiceFormData) {
      try {
        await systemConfigService.systemConfigService.updateSystemConfig(systemConfigServiceFormData);
  ...
  const { data: frequentlyAskedQuestionsData, refetch: refetchFrequentlyAskedQuestionsData } = useInfiniteQuery(
    [QUERY_KEY.FREQUENTLY_ASKED_QUESTION],
    async () => {
  ...
  const handleCreateFAQs = async (data: FrequentlyAskedQuestion) => {
    if (data) {
      try {
        await systemConfigService.frequentlyAskedQuestionService.createFrequentlyAskedQuestion(data);
  ...
  const handleUpdateFAQs = async (data: FrequentlyAskedQuestion) => {
    if (data) {
      try {
        await systemConfigService.frequentlyAskedQuestionService.updateFrequentlyAskedQuestion(data);
    ...
    if (frequentlyAskedQuestionsData) {
      if (
        ...
        if (listFrequentlyAskedQuestionsCanBeDelete) {
          const listFrequentlyAskedQuestionsID = listFrequentlyAskedQuestionsCanBeDelete.map(({ id }) => id);
  ...
  useEffect(() => {
    if (systemConfigsData?.pages[0].moneyToPointPercent || systemConfigsData?.pages[0].onePointToMoney) {
      const systemConfigsDataEffect = systemConfigsData?.pages[0];
      setValueMoneyToPointPercent(
        systemConfigsDataEffect?.moneyToPointPercent ? Number(systemConfigsDataEffect?.moneyToPointPercent) : 0,
      );
      setValueOnePointToMoney(
        systemConfigsDataEffect?.onePointToMoney ? Number(systemConfigsDataEffect?.onePointToMoney) : 0,
      );
    }
  }, [systemConfigsData]);
              ...
              <div className='p-7'>
                ...
                >
                  ...
                  >
                    ...
                    <Row gutter={2}>
                      <Col xs={12}>
                        <Typography.Paragraph>Phần trăm (%) số tiền đơn hàng được quy đổi ra điểm</Typography.Paragraph>
                        <Form.Item
                          name='moneyToPointPercent'
                          rules={[
                            {
                              required: true,
                              message: 'Phần trăm (%) số tiền đơn hàng được quy đổi ra điểm không được để trống',
                            },
                          ]}
                        >
                          <InputNumber
                      ...
                      <Col xs={12}>
                        ...
                        <div className='text-[#999]'>
                          <span>{`Ví dụ: Tỉ lệ quy đổi điểm sang VND là 1:${valueOnePointToMoney}`}</span> <br />
                            ...
                            >
                              <div style={{ width: '95%' }}>
                                <Form.Item
                                  {...restField}
                                  name={[name, 'reason']}
                                  rules={[{ required: true, message: 'Lí do không được bỏ trống' }]}
                                >
          ...
          {frequentlyAskedQuestionsData && (
            <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
              <div className='border-b border-stroke py-4 px-7 dark:border-strokedark'>
                <h3 className='font-medium text-black dark:text-white text-[16px]'>Câu hỏi thường gặp</h3>
              </div>
              <div className='p-7'>
                <Form
                  ...
                  autoComplete='off'
                  initialValues={{ frequentlyAskedQuestions: frequentlyAskedQuestionsData.pages[0] }}
                  style={{ maxWidth: '100%' }}
                >
