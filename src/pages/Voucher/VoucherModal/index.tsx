...
const VoucherModal = ({
  ...
  const { data: product, isLoading: isLoadingProduct } = useInfiniteQuery(
    ...
    async () => {
      const params = {
        pageIndex: onGetPagination.current,
        pageSize: onGetPagination.pageSize,
        name: searchProduct,
      };
    ...
    if (voucherDate.endDate === "" || errors.endDate?.type === "required") {
      ...
      const startDateFormat =
        modalType === ModalType.CREATE
          ? moment(voucherDate.startDate, DATE_FORMAT_DDMMYYYYTHHMMSS).format(
              DATE_FORMAT_YYYYMMDDTHHMMSS,
            )
          : voucherDate.startDate;
      const endDateFormat =
        modalType === ModalType.CREATE
          ? moment(voucherDate.endDate, DATE_FORMAT_DDMMYYYYTHHMMSS).format(
              DATE_FORMAT_YYYYMMDDTHHMMSS,
            )
          : voucherDate.endDate;
      newData = {
        ...
        <form id="form-voucher" onSubmit={handleSubmit(onSubmit)}>
          <>
            ...
            showVoucherApplyForProductTable ? (
              ...
              <div className="mt-7">
                {modalType === ModalType.INFORMATION &&
                  voucherById?.saleScope === SaleScope.BY_PRODUCT && (
                    <>
                      <Typography.Text
                        type="secondary"
