...
const Materials = () => {
  ...
  const {
    ...
    refetch,
  } = useInfiniteQuery([QUERY_KEY.MATERIALS, fromDateTimeDebounce, toDateTimeDebounce], async () => {
    const params = {
      pageIndex: 0,
      pageSize: 10,
    ...
    try {
      if (id && typeModal) {
        ...
        if (Object.keys.length > 0) {
          setMaterialModal({ hasShow: true, materialDetail: materialDetail, modalType: typeModal });
        }
      }
    } catch (error) {
      console.log(error);
