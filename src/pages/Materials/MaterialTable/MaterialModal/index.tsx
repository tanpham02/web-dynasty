...
const MaterialModal = ({
  ...
  const { fields, append, remove } = useFieldArray({ name: 'materialInfo', control });
  ...
  const onSubmitData = async (dataSubmit: Material) => {
    ...
    try {
      ...
      onCloseModal?.();
      refetch();
      toast.success(`${modalType === ModalType.CREATE ? 'Thêm' : 'Cập nhật'} nguyên liệu thành công`, {
  ...
  const handleAddProductInfoImport = (field: any, index: any) => {
    append({ name: field?.[index]?.name, price: field?.[index]?.price, quantity: field?.[index]?.quantity });
  };
              ...
              {modalType !== ModalType.INFORMATION && (
                <Col span={12}>
                    ...
                    render={({ field: { value } }) => (
                      <DatePicker
                        className={`!py-[7px] w-full border-solid border rounded-md ${
                          errors.importDate?.type === 'required' ? 'border  !border-danger' : 'border-current'
                        } blur-0 `}
                  ...
                  {errors?.importDate?.type === 'required' && (
                    <small className='text-danger text-[13px]'>Ngày nhập hàng không được rỗng</small>
                  )}
                </Col>
              )}
