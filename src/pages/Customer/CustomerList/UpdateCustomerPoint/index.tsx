...
export default function UpdateCustomerPointModal({
  ...
  const { data: customerHistoryData, refetch: refreshCustomerHistoryData } =
    useInfiniteQuery(
  ...
  const COLUMNS: TableColumn[] = [
    ...
    },
    {
      ...
      align: "center",
      render: (_, record) => (
        <span>{`${record.type === CalculatorPoint.ADDITION ? "+" : "-"} ${
          record.point
        }`}</span>
      ),
    },
          ...
          <Col span={12}>
            <div className="flex items-center">
              ...
              <button
                ...
                onClick={(e: any) => {
                  e.preventDefault();
                  setVisibleModalUpdateCustomerPoint(
                    !visibleModalUpdateCustomerPoint,
                  );
                }}
              >
                <EditOutlined
