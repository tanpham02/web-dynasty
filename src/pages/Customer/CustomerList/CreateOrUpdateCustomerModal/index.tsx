...
import { Button, Col, DatePicker, Input, Modal, Row, Select, Table, TablePaginationConfig, Typography } from 'antd';
...
const COLUMNS: TableColumn[] = [
  ...
  },
  {
    ...
    align: 'center',
    render: (_, record) => (
      <span>{record.createdDate ? formatDate(record.createdDate, DATE_FORMAT_DDMMYYYY) : ''}</span>
    ),
  },
  {
    title: 'Số điểm',
...
export default function CreateOrUpdateCustomerModal({
  ...
  const { cities, districts, wards } = useShippingLocation(location.cityID, location.districtID);
  ...
  const pagination = useMemo(() => {
    const current = customerHistoryData?.pages[customerHistoryData.pages.length - 1]?.pageable.pageNumber;
    const total = customerHistoryData?.pages[customerHistoryData.pages.length - 1]?.totalElements;
    const pageSize = customerHistoryData?.pages[customerHistoryData.pages.length - 1]?.pageable.pageSize;
  ...
  const handleUpdateLocation = (typeLocation: LocationEnum, locationID: number | string) => {
    if (locationID)
      switch (typeLocation) {
        case LocationEnum.CITY:
          // eslint-disable-next-line no-case-declarations
          const citySelected = cities?.find((city) => city.value === locationID);
        ...
        case LocationEnum.DISTRICT:
          // eslint-disable-next-line no-case-declarations
          const districtSelected = districts?.find((district) => district.value === locationID);
  ...
  return (
    <>
      <Modal
        ...
        title={typeModal === 'UPDATE' ? 'Cập nhật thông tin khách hàng' : 'Thông tin chi tiết của khách hàng'}
      ...
      >
          ...
          <Col className='mt-5 lg:w-[40%] md:w-[100%] xsm:w-[100%] 2xsm:w-[100%]'>
            {typeModal !== 'CREATE' && (
              <Row className='mt-1'>
                <Col span={12}>
                  <Typography>
                    Mã ID Khách hàng: <span className='font-bold'>{customerDetail?.id}</span>
                  </Typography>
                </Col>
              </Row>
            )}
            ...
            >
              <Col span={12}>
                <Typography>
                  Tên khách hàng:
                  {typeModal === 'DETAIL' && <span className='font-bold'> {customerDetail?.fullName}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Input
                            className={` border-solid  border-[1px] ${errors.fullName ? '!border-danger' : ''}`}
                            value={value}
                            onChange={onChange}
                          />
                        )}
                      ...
                      {errors.fullName?.type === 'required' ? (
                        <small className='text-danger'>Tên khách hàng không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
                ...
                <Col span={12}>
                  <Typography className='flex flex-wrap'>
                    ...
                    <div className='flex flex-wrap'>
                      <span
                        className='font-bold '
                        style={{ color: customerDetail?.membershipDTO?.color, marginLeft: '5px' }}
                      >
                ...
                <Col span={12}>
                  <Typography>
                    ...
                    <span className='font-bold'>
                      {customerDetail?.totalAmountPurchased && formatCurrencyVND(customerDetail?.totalAmountPurchased)}
                    </span>
                  </Typography>
                </Col>
              </Row>
            ...
            >
              <Col span={12}>
                <Typography>
                  Số điện thoại :{' '}
                  {typeModal === 'DETAIL' && <span className='font-bold'>{customerDetail?.phoneNumber}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Input
                            className={` border-solid  border-[1px] ${errors.phoneNumber ? '!border-danger' : ''}`}
                            value={value}
                            onChange={onChange}
                          />
                        )}
                      ...
                      {errors.phoneNumber?.type === 'required' && (
                        <small className='text-danger'>Số điện thoại không được rỗng</small>
                      )}
                      {errors.phoneNumber?.type === 'pattern' && (
                        <small className='text-danger'>Số điện thoại không đúng định dạng</small>
                      )}
                    </>
                  )}
                </Typography>
              ...
              <Col span={12}>
                <Typography>
                  ...
                  {typeModal === 'DETAIL' && (
                    <span className='font-bold'>
                      {customerDetail?.birthday && moment(customerDetail?.birthday).format(DATE_FORMAT_DDMMYYYY)}
                    </span>
                  )}
                  {typeModal !== 'DETAIL' && (
                    <>
                      ...
                      {errors.birthday?.type === 'required' ? (
                        <small className='text-danger'>Ngày sinh không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
            ...
            <Row className='mt-4'>
              <Col span={12}>
                <Typography>
                  Tỉnh/Thành phố: {typeModal === 'DETAIL' && <span className='font-bold'>{customerDetail?.city}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value } }) => (
                          <Select
                            ...
                            defaultValue={0}
                            style={{ width: '100%' }}
                            onChange={(value) => handleUpdateLocation(LocationEnum.CITY, value)}
                            options={cities}
                          />
                        )}
                      />
                      {errors.cityId?.type === 'required' ? (
                        <small className='text-danger'>Tỉnh/Thành phố không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
            ...
            <Row className='mt-4'>
              <Col span={12}>
                <Typography>
                  Quận/Huyện: {typeModal === 'DETAIL' && <span className='font-bold'>{customerDetail?.district}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value } }) => (
                          <Select
                            ...
                            value={value}
                            style={{ width: '100%' }}
                            onChange={(value) => handleUpdateLocation(LocationEnum.DISTRICT, value)}
                            options={districts}
                          />
                        )}
                      />
                      {errors.districtId?.type === 'required' ? (
                        <small className='text-danger'>Quận/Huyện không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
            ...
            <Row className='mt-4'>
              <Col span={12}>
                <Typography>
                  Xã/Phường: {typeModal === 'DETAIL' && <span className='font-bold'>{customerDetail?.ward}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value } }) => (
                          <Select
                            ...
                            value={value}
                            style={{ width: '100%' }}
                            onChange={(value) => handleUpdateLocation(LocationEnum.WARD, value)}
                            options={wards}
                          />
                        )}
                      />
                      {errors.wardId?.type === 'required' ? (
                        <small className='text-danger'>Xã/Phường không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
            ...
            <Row className='mt-4'>
              <Col style={{ width: '100%' }}>
                <Typography>
                  Số nhà cụ thể: :{' '}
                  {typeModal === 'DETAIL' && <span className='font-bold'>{customerDetail?.address}</span>}
                  {typeModal !== 'DETAIL' && (
                    <>
                      {' '}
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Input
                            className={` border-solid  border-[1px] ${errors.address ? '!border-danger' : ''}`}
                            value={value}
                            style={{ width: '100%' }}
                            onChange={onChange}
                      ...
                      {errors.address?.type === 'required' ? (
                        <small className='text-danger'>Số nhà cụ thể không được rỗng</small>
                      ) : (
                        ''
                      )}
                    </>
        ...
        {customerHistoryData?.pages && customerHistoryData?.pages?.length > 0 && typeModal === 'DETAIL' && (
          <Table
            rowKey='id'
