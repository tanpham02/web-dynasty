...
import { FormContextCKEditor, FormContextInput } from '~/components/NextUI/Form';
...
const OtherConfig = () => {
  const [tabKeys, setTabKeys] = useState<TabKeys | string | number>(TabKeys.FAQS)

  console.log("key:", tabKeys)

  const renderCardBody = useMemo(() => {
    if (tabKeys === TabKeys.FAQS)
      return <FAQsTab />
    else return <FormContextCKEditor key={tabKeys} name={`termAndPolicy.${tabKeys}`} />
  }, [tabKeys])

  return (
    <Card className="col-span-2">
      <CardHeader>
        <Tabs selectedKey={tabKeys} onSelectionChange={setTabKeys} size="md" variant="underlined" color='primary' aria-label="Tabs sizes" >
          ...
          <Tab key={TabKeys.TERM_AND_CONDITION} title="Điều khoản và điều kiện" />
        </Tabs>
      </CardHeader>
      <Divider />
      <CardBody className={`p-4 ${tabKeys === TabKeys.FAQS && "grid grid-cols-3 gap-4"}`}>
        {renderCardBody}
      </CardBody>
    </Card>
  );
